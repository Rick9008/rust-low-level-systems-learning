...
// mask is for head & mask to get actual index
struct SpscRing<T> {
...
impl<T> SpscRing<T> {
    ...
    fn write_slot(&self, idx: usize, item: T) {
        unsafe {(*self.buf[idx & self.mask].get()).write(item);}
    }
...
fn Channel<T>(cap: usize) -> (Producer<T>, Consumer<T>) {
    let ring = Arc::new(SpscRing::<T>::new(cap));
    (Producer {
        ring: ring.clone(),
    }, Consumer {
            ring
