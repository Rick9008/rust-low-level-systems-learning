...
//! [`crate::executor::block_on`] 就能跑)。
//!
//! ## [Abstract]
//! 一個變換打通全部:**blocking 原語 = 睡執行緒的佇列 + park/unpark;
//! async 原語 = 存 `Waker` 的佇列 + `Pending`/`wake`**。
//! 這是 repo「rendezvous 三部曲」的第三章:
//! 1. [`crate::thread_pool::JobHandle`]——one-shot,condvar 睡
//! 2. [`crate::file_io_offload::JoinFuture`]——one-shot,waker 睡
...
//!   (存一張、不累積)——正是 [`crate::executor`] park/unpark 的 token
...
mod tests {
    use super::*;
    use crate::executor::{Delay, YieldNow, block_on};
    use std::sync::Arc;
