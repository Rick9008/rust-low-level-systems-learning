...
## dbg!/451 根因鏈定版(8/9 深夜,Withers 對帳)

pipe 斷的真實鏈:**/var/log/systemd 全機服務共用 → 誰都可能灌向 400MB 上限 → 撞上限 logger 重啟 →
重啟弄斷 stderr pipe**。「restarted」與 400MB 是同一鏈的兩節,稿原句只講了後半。
⚠ 不是 dbg! 自己灌爆(資料夾共用)——**別講成自殺循環**,那是敘事重力(這次是 Claude 的)。
共用資料夾版本反而強化「看起來隨機」:任何服務都能把陷阱上膛。稿與上場包已改。
