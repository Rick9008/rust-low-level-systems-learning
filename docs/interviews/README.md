...
| 8/3 一(大塊) | 開機:**sim o lite**(`drills/src/ds/boot_planner.rs` 填空 25m——8/2 深夜新增的 algo 系首題:Kahn 分層+最長路徑+環回報;圖論興趣槽兼 n 的暖身,取代原 graph 一題)→ n **lite**(自 8/9 前移;認題輪取消,被 lite 覆蓋)+ **l lite(自 8/2 滑入;HW-L 卡當前導;若 8/2 深夜已打則此格改複讀 15m)** + i–k 洞複掃 + 骨架默寫抽查 15m + **卡 FP、卡 TQ**(8/2 晚定的認題卡復活輪:白天上班空檔各 15–20m;FP=c framer 肌肉、TQ=timer,正好接 watchdog 的 deadline 課) | (選)clarify 句庫一類唸 3 遍 |
| 8/4 二(大塊) | **背靠背模擬**:Claude 出全新題 ×2(禁區同 live sim 規:不碰 j–n hidden spec 域),各 25–30m 只跑到 state 表+骨架,**中間休 15 分鐘照 8/6 真實節奏**——兩場連打的體力與切換先試一次;**每題埋一個 sizing 小題,用量級五行頭(Given/Chain/Cross/Sanity/Verdict)作答**(7/31 reps v5 定案的結帳骨架);下午空檔認題卡場:**FR → AG-R → TA → AG-T**(砍序照這個順序反過來砍;FR=e2 fd_registry=JD sleeper、AG-R=widest path 認題=algo 系、TA=f aggregator、AG-T=聚合樹修復;收完卡全結,8/5 翻閱只剩複讀) | **08:30 起(梯度開始;9:15 場只需 07:45 起,比 R1 的 8:45 場輕)** |
