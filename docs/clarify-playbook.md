...
## 五問結帳表(7/21 卡#4/#6 批改沉澱:問了 ≠ 結清)

前置動作(讀題稅):題幹**每一句話**先標「餵哪一問」再開始問——
7/21 兩卡漏的全是題幹白給的句子("seconds to minutes" → 容量因子二;
"must not hammer" → 限流)。

| 問 | 問了(類別分) | 結清(結帳條件) |
|---|---|---|
| 掉不掉 | 列出 drop 分支 | **裁決句**:選哪個 full policy + 哪兩個約束相乘逼出它 |
| 速率/規模 | 要到數字 | 數字**活進算式**,結果過荒謬檢查(荒謬 → 回頭砍假設) |
| 容量 | 問到 buffer 大小 | **兩因子乘法**:rate × 要撐的時長——缺一因子 = 沒問 |
| SLA | 問到快慢軸 | 一句**數字承諾**("within X s"),X 拆成你選的參數 |
| 偵測 | 問到怎麼知道死 | 判死定義 + **去抖(連續 N 次)** + 窗口公式 |

檢查口訣:每一問收尾,手上要多出**一個數字、一條式子、或一句裁決**——
三者皆無,就是問了沒結帳。
