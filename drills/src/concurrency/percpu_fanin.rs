...
fn take_ch(chs: &[Arc<Chan>], new_records: &mut Vec<Record>, budget: usize) {
    for ch in chs {
        for _ in 0..budget {
            match ch.try_pop() {
                Some(rec) => new_records.push(rec),
                None => break,
            }
        }
    }
...
pub fn aggregator_loop(
    ...
    loop {
        {
            out.lock().unwrap().extend(new_records.drain(..));
        }
        take_ch(chs, &mut new_records, budget);
...
mod tests {
    ...
    #[test]
    fn shutdown_exits_promptly_without_pending_wake() {
        let chs = make_channels(3, 64);
        let waker = Waker::new();
        let stop = AtomicBool::new(true);
        let out = Mutex::new(Vec::new());
        let dropped = AtomicU64::new(0);
        for (core, _ch) in chs.iter().enumerate() {
            for seq in 0..10 {
                let _ = chs[core].try_push(Record { core, seq });
            }
        }
